...
case $job_type in
    ...
    mapr)
       env ubuntu_mapr_image_name=${mapr_image} SIM_REPO_PATH=$WORKSPACE tox -e venv -- sahara-image-create -p mapr -i ubuntu
       check_error_code $? ${mapr_image}.qcow2
       upload_image "mapr" "ubuntu" ${mapr_image}
