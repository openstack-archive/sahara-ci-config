    ...
    vanilla*)
       ...
       if [ "${image_type}" == 'centos' ]; then
           ...
           if [ "$image_type" == "fedora" ]; then
              ...
              # we using the same version of fedora that is in DIB
              FEDORA_URL="http://172.18.168.44/images/fedora.x86_64.qcow2"
           fi
       fi
       ...
       case $hadoop_version in
           1)
              sudo ${image_type}_vanilla_hadoop_1_image_name=${vanilla_image} DIB_CLOUD_IMAGES=$FEDORA_URL JAVA_DOWNLOAD_URL='http://127.0.0.1:8000/jdk-7u51-linux-x64.tar.gz' SIM_REPO_PATH=$WORKSPACE bash -x diskimage-create/diskimage-create.sh -p vanilla -i $image_type -v 1
              ...
              ;;
           2.4)
              sudo ${image_type}_vanilla_hadoop_2_4_image_name=${vanilla_two_four_image} DIB_CLOUD_IMAGES=$FEDORA_URL JAVA_DOWNLOAD_URL='http://127.0.0.1:8000/jdk-7u51-linux-x64.tar.gz' SIM_REPO_PATH=$WORKSPACE bash -x diskimage-create/diskimage-create.sh -p vanilla -i $image_type -v 2.4
              ...
              ;;
           2.6)
              sudo ${image_type}_vanilla_hadoop_2_6_image_name=${vanilla_two_six_image} DIB_CLOUD_IMAGES=$FEDORA_URL JAVA_DOWNLOAD_URL='http://127.0.0.1:8000/jdk-7u51-linux-x64.tar.gz' SIM_REPO_PATH=$WORKSPACE bash -x diskimage-create/diskimage-create.sh -p vanilla -i $image_type -v 2.6
