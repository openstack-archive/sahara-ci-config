...
case $plugin in
    ...
    ;;
    storm_1.0.1)
       env ubuntu_spark_image_name=${storm_1.0.1_image} SIM_REPO_PATH=$WORKSPACE tox -e venv -- sahara-image-create -p storm -s 1.0.1
       check_error_code $? ${storm_1_0_1_image}.qcow2
       upload_image "${plugin}" "${username}" ${storm_1_0_1_image}
       scenario_conf_file="$sahara_templates_path/storm_1.0.1.yaml.mako"
