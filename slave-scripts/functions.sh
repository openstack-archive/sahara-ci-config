...
enable_pypi() {
  ...
  mv $sahara_configs_path/pip.conf ~/.pip/pip.conf
  mv $sahara_configs_path/.pydistutils.cfg ~/.pydistutils.cfg
}
...
write_tests_conf() {
  ...
  [ "$USE_NEUTRON" == "true" ] && NETWORK="neutron" && TENANT_ID=$NEUTRON_LAB_TENANT_ID
  [ "$USE_NEUTRON" == "false" ] && NETWORK="nova-network" && TENANT_ID=$NOVA_NET_LAB_TENANT_ID
...
run_tests() {
  ...
  if [[ "$JOB_NAME" =~ scenario ]]; then
      ...
      tox -e integration -- $plugin --concurrency=$concurrency || failure "Integration tests are failed"
  fi
}
