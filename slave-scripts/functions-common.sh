...
start_sahara() {
  ...
  if ! timeout ${api_responding_timeout} sh -c "while ! curl -s http://127.0.0.1:8386/v1.1/ 2>/dev/null | grep -q 'The request you have made requires authentication.' ; do sleep 1; done"; then
