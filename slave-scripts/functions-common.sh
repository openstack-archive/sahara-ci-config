...
run_tests() {
  ...
  cd $SAHARA_TESTS_PATH
  if [ "$ZUUL_BRANCH" == "stable/kilo" ]; then
    scenario_credentials=${scenario_credentials%.*}
    scenario_config=${scenario_config%.*}
    tox -e venv -- scenario $scenario_credentials $scenario_edp $scenario_config | tee tox.log
  else
    ...
    tox -e venv -- scenario --verbose -V $template_vars_file $scenario_credentials $scenario_edp $scenario_config | tee tox.log
  fi
